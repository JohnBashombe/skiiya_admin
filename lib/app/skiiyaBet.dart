...
class _SkiiyaBetState extends State<SkiiyaBet> {
  ...
  filterChampionships(String champ, String country, int pos) {
    int index = loadSideDataArrayChamp.indexOf(champ);
    // print('Serie A index: $index');
    // MAKE SURE WE HAVE ENOUGH CONTENT IN ARRAY
    if (loadSideDataArrayChamp.length > 1) {
      loadSideDataArrayChamp.removeAt(index);
      loadSideDataArrayCountry.removeAt(index);
      // print('Cond checked');
      loadSideDataArrayChamp.insert(pos, champ);
      loadSideDataArrayCountry.insert(pos, country);
    } else {
      // print('Else Cond checked');
      // OTHERWISE ADD IT AT THE LAST POSITION IN THE ARRAY
      loadSideDataArrayChamp.removeAt(index);
      loadSideDataArrayCountry.removeAt(index);
      loadSideDataArrayChamp.insert(loadSideDataArrayChamp.length - 1, champ);
      loadSideDataArrayCountry.insert(
          loadSideDataArrayCountry.length - 1, country);
    }
  }
